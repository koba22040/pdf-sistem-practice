...
embedObjects(pdfBlob, object) {
  if (!pdfBlob || pdfBlob.toString() !== "Blob") {
    throw new Error("Please set the PDF blob as the first argument.");
  }
  if (!object || typeof object !== "object") {
    throw new Error("Please set a valid object for embedding.");
  }
  ...
  return new Promise(async (resolve, reject) => {
    try {
      ...
      // 既定フォントの設定
      if (self.useCustomFont && self.useCustomFont.toString() == "Blob") {
        self._defaultFont = new Uint8Array(self.useCustomFont.getBytes());
      } else if (self.useStandardFont && typeof self.useStandardFont == "string") {
        self._defaultFont = self.PDFLib.StandardFonts[self.useStandardFont];
      }

            // 既存PDFを読み込み
      const pdfData = await self.getPDFObjectFromBlob_(pdfBlob).catch(err => reject(err));
      // object を更新（フォントなどの処理）
      const { updatedObject, customFontCheck } = self.updateObject_(object);

      const embeddedFonts = {}; // 埋め込み済みフォントを保持するオブジェクト

      // 全てのページオブジェクトを調べて、使用するフォントを先に全て埋め込む
      for (const key in updatedObject) {
          const drawList = updatedObject[key];
          if (drawList) {
              for (const o of drawList) {
                  if (o.text) {
                      const fontName = o.fontName || "NotoSansJP"; // 設定ファイルから取得するかデフォルト
                      
                      // 既に埋め込まれていなければ、ここで埋め込む
                      if (!embeddedFonts[fontName]) {
                          const fontBytes = self._fontMap && self._fontMap[fontName] 
                                            ? self._fontMap[fontName] 
                                            : self._defaultFont;

                          if (fontBytes) {
                              embeddedFonts[fontName] = await pdfDoc.embedFont(fontBytes);
                              Logger.log(`フォント "${fontName}" を埋め込みました。`);
                          } else {
                              Logger.log(`フォント "${fontName}" が見つかりません。`);
                          }
                      }
                      
                      // 次の描画ループで使用できるように、テキストオブジェクトに埋め込み済みフォントオブジェクトを設定
                      o.font = embeddedFonts[fontName];
                  }
              }
          }
      }
      ...
      Logger.log("=== embedObjects START ===");
      Logger.log("object keys: " + JSON.stringify(Object.keys(object)));

      // 各ページへの描画
      for (let i = 0; i < pages.length; i++) {
        const page = pages[i];
        const drawList = updatedObject[`page${i + 1}`];
        ...
        if (drawList) {
          for (const o of drawList) {
            ...
            if (o.text) {
              // フォント処理
              // if (o.fontName && self._fontMap && self._fontMap[o.fontName]) {
              //   o.font = await pdfDoc.embedFont(self._fontMap[o.fontName]);
              // } else if (self._defaultFont) {
              //   o.font = await pdfDoc.embedFont(self._defaultFont);
              // }

              page.drawText(o.text, {
                  ...o, // o.x, o.y, o.size など
                  font: o.font, // 事前処理でキャッシュした埋め込み済みフォントオブジェクト
              });
              continue;
            }
            ...
            if (o.type === "ellipse" || o.type === "circle") {
              Logger.log(">>> ellipse draw " + JSON.stringify(o));
              ...
              page.drawEllipse({
                ...
                xScale: o.width / 2,
                yScale: o.height / 2,
                borderColor: self.PDFLib.rgb(0, 0, 0),
      ...
      const bytes = await pdfDoc.save();
      resolve(
        Utilities.newBlob([...new Int8Array(bytes)], MimeType.PDF, `new_${pdfBlob.getName()}`)
      );
  ...
  updateObject_(object) {
    ...
    Logger.log(">>> updateObject_ input: textCount=" 
           + object.page1.filter(o => o.text).length 
           + " ellipseCount=" 
           + object.page1.filter(o => o.type === "ellipse").length);

    //Logger.log(">>> updateObject_ output: " + JSON.stringify(updatedObject));
  ...
  async getPDFObjectFromBlob_(blob) {
    ...
    /**塗りつぶし円
   * ### Description
   * Draw an ellipse (or circle) on PDF using pdf-lib.
   * 
   * @param {Object} pdfBlob Blob of PDF data.
   * @param {Object} options Object with ellipse properties.
   * Example:
   * {
   *   page: 1,
   *   x: 200,
   ...
   * @return {promise} PDF Blob.
   */
  async drawEllipse(pdfBlob, options) {
    if (!options || typeof options !== "object") {
      throw new Error("Please set an options object for drawing ellipse.");
    }
    ...
    return new Promise(async (resolve, reject) => {
      try {
        ...
        const targetPage = options.page ? options.page - 1 : 0;
        const page = pages[targetPage];
        const { width, height } = page.getSize();

        // 色設定
        const strokeColor = options.color || [0, 0, 0];
        const fillColor = options.fillColor || null;
        const lineWidth = options.borderWidth || 1;

        const drawOptions = {
          x: options.x || width / 2,
          y: options.y || height / 2,
          xScale: options.width || 50,
          yScale: options.height || 50,
          borderWidth: lineWidth,
          color: self.PDFLib.rgb(...strokeColor),
        };

        if (fillColor) {
          drawOptions.color = self.PDFLib.rgb(...fillColor);
          drawOptions.borderColor = self.PDFLib.rgb(...strokeColor);
        }
        ...
        pdfDoc.addPage(page);
        const bytes = await pdfDoc.save();
        const newBlob = Utilities.newBlob([...new Int8Array(bytes)], MimeType.PDF, `ellipse_${pdfBlob.getName()}`);
        resolve(newBlob);
      } catch (err) {
        reject(err);
      }
  ...
  /**枠のみ円
 * Draw a circle with black border (no fill).
 * For a transparent circle with only black outline.
 */
/**
 * Draw a circle or ellipse with black border (no fill).
 * Supports width and height.
 */
/**
 * Draw a circle or ellipse with black border (no fill).
 * Supports width/height and transparency.
 */
async drawCircle(pdfBlobOrOptions, maybeOptions) {
  let pdfBlob, options;
  if (pdfBlobOrOptions && typeof pdfBlobOrOptions.toString === 'function' && pdfBlobOrOptions.toString() === 'Blob') {
    pdfBlob = pdfBlobOrOptions;
    options = maybeOptions || {};
  } else {
    options = pdfBlobOrOptions || {};
    pdfBlob = this.pdfBlob;
  }

  if (!pdfBlob) throw new Error("No PDF blob. Call setPDFBlob() or pass a blob to drawCircle().");

  const self = this;
  return new Promise(async (resolve, reject) => {
    try {
      const src = await self.getPDFObjectFromBlob_(pdfBlob);
      const dst = await self.PDFLib.PDFDocument.create();
      const pages = await dst.copyPages(src, src.getPageIndices());
      const idx = (options.page ? options.page - 1 : 0);
      ...
      page.drawEllipse({
        x: options.x ?? page.getWidth() / 2,
        y: options.y ?? page.getHeight() / 2,
        xScale: width / 2,
        yScale: height / 2,
        borderColor: self.PDFLib.rgb(0, 0, 0), // 黒線
        borderWidth: options.borderWidth ?? 1.5,
      });

      for (let i = 0; i < pages.length; i++) dst.addPage(pages[i]);
      const bytes = await dst.save();
      const out = Utilities.newBlob([...new Int8Array(bytes)], MimeType.PDF, `circle_${pdfBlob.getName()}`);
      resolve(out);
    } catch (err) {
      reject(err);
    }
...
/** * Register multiple fonts for later use by font name. * @param {Object} fontMap - { "NotoSansJP": Blob, "NotoSerifJP": Blob } */ 
PDFApp.prototype.registerFontMap = function(fontMap) { this._fontMap = {}; for (const [name, blob] of Object.entries(fontMap)) { this._fontMap[name] = new Uint8Array(blob.getBytes()); } };