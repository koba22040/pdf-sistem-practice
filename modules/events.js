...
import { openAutoFillModal, updateFloatingControls, cancelGroupingMode, openGroupingModal, updateAllUnassignedSelectors, updateValueInputForm } from './ui.js';
...
function handleMouseMove(e) {
    ...
    if (state.ui.currentMode !== 'placement' || state.ui.groupingModeType) {
        drawCanvasWithBoxes(); 
        return;
    }
...
function handleKeyDown(e) {
    ...
    if ((e.metaKey || e.ctrlKey) && e.key === 'z') {
        e.preventDefault();
        // undo(); // This will be in history.js
        return;
    }
    ...
    if (state.ui.currentMode === 'placement' && (e.key === 'Delete' || e.key === 'Backspace')) {
        ...
        if (state.ui.selectedFieldIds.length > 0) {
            if (confirm(`選択中の ${state.ui.selectedFieldIds.length} 個の項目を削除しますか？`)) {
                saveStateToHistory();
