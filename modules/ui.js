...
export function initUI() {
    ...
    const ids = [
        ...
        'af-preset-select', 'af-load-preset-btn', 'af-save-preset-btn', 'af-preset-name', 'af-delete-preset-btn',
    ...
    DOMElements.afSavePresetBtn?.addEventListener('click', saveAutoFillPreset);
    DOMElements.afLoadPresetBtn?.addEventListener('click', applyAutoFillPreset);
    DOMElements.afDeletePresetBtn?.addEventListener('click', deleteAutoFillPreset);
    DOMElements.afSourceSelect?.addEventListener('change', () => {
        const currentRules = [];
        DOMElements.afRulesContainer.querySelectorAll('.rule-row').forEach(row => {
            currentRules.push({ key: row.querySelector('.af-key').value, val: row.querySelector('.af-val').value });
        });
        DOMElements.afRulesContainer.innerHTML = '';
        if (currentRules.length > 0) currentRules.forEach(r => addAutoFillRuleRow(r.key, r.val));
        ...
        if (item.type === 'group') {
            ...
            q.choices.forEach(c => {
                ...
                input.addEventListener('change', (e) => {
                    ...
                    runAutoFillEngine(c.fieldId, e.target.checked);
                    drawCanvasWithBoxes();
                });
        ...
        state.ui.selectedFieldIds.forEach(id => {
            ...
            <select data-field-id="${id}">
                ...
                <optgroup label="通常入力 (1つの枠)">
                    <option value="year-ad">年（西暦）</option><option value="year-wareki">年（和暦）</option><option value="month">月</option><option value="day">日</option><option value="hour-24">時 (0-23)</option><option value="hour-am">時 (午前)</option><option value="hour-pm">時 (午後)</option><option value="minute">分 (0-59)</option><option value="minute-am">分 (午前のみ)</option><option value="minute-pm">分 (午後のみ)</option>
                </optgroup>
                ...
                <optgroup label="自動入力">
                    ...
                    <option value="today-hour">【自動】今日の時 (24h)</option><option value="today-minute">【自動】今日の分</option>
                </optgroup>
            </select>`;
            
            listItem.addEventListener('mouseenter', () => { state.ui.hoveredFieldId = id; drawCanvasWithBoxes(); });
            listItem.addEventListener('mouseleave', () => { state.ui.hoveredFieldId = null; drawCanvasWithBoxes(); });
...
export function openAutoFillModal(targetId) {
    ...
    state.questions.forEach(q => {
        const option = document.createElement('option'); option.value = q.id; option.textContent = `【質問】${q.title}`; DOMElements.afSourceSelect.appendChild(option);
    });
...
function addAutoFillRuleRow(keyVal, valueVal) {
    const sourceId = DOMElements.afSourceSelect?.value;
    const sourceField = state.fieldPositions[sourceId];
    const sourceQuestion = state.questions.find(q => q.id === sourceId);
    const isBooleanSource = sourceField && (sourceField.type === 'circle' || sourceField.type === 'check');

    const row = document.createElement('div'); row.className = 'rule-row';
    
    let keyInputHtml;
    if (sourceQuestion) {
        keyInputHtml = `<select class="af-key" style="flex: 1; padding: 5px; border: 1px solid #ccc; border-radius: 4px;">`;
        sourceQuestion.choices.forEach(c => {
            keyInputHtml += `<option value="${c.name}" ${keyVal === c.name ? 'selected' : ''}>${c.name}</option>`;
        });
        keyInputHtml += `</select>`;
    } else if (isBooleanSource) {
        keyInputHtml = `
            <select class="af-key" style="flex: 1; padding: 5px; border: 1px solid #ccc; border-radius: 4px;">
                <option value="true" ${keyVal === 'true' ? 'selected' : ''}>選択されている (true)</option>
                <option value="false" ${keyVal === 'false' ? 'selected' : ''}>選択されていない (false)</option>
            </select>`;
    } else {
        keyInputHtml = `<input type="text" class="af-key" placeholder="もし(例:1)" value="${keyVal || ''}">`;
    }

    row.innerHTML = `${keyInputHtml}<span>→</span><input type="text" class="af-val" placeholder="これ(例:工学部)" value="${valueVal || ''}"><span class="rule-delete">×</span>`;
...
export function saveToLocalStorage() {
    const data = {
        ...
        elementCounter: state.data.elementCounter,
        timestamp: Date.now()
    };
    localStorage.setItem('pdf_app_backup', JSON.stringify(data));
    console.log("Saved to localStorage");
}

export function loadFromLocalStorage() {
    const dataStr = localStorage.getItem('pdf_app_backup');
    if (!dataStr) return false;
    try {
        const data = JSON.parse(dataStr);
        state.fieldPositions = data.fieldPositions || {};
        state.questions = data.questions || [];
        state.data.elementCounter = data.elementCounter || 1;
        ...
        if (state.questions.length > 0) {
            ...
            state.questions.forEach(q => renderQuestionBlock(q));
        }
        return true;
    } catch (e) {
...
function loadAutoFillPresets() {
    const select = DOMElements.afPresetSelect;
    if (!select) return;
    select.innerHTML = '<option value="">-- 保存済みセットを選択 --</option>';
    const presets = JSON.parse(localStorage.getItem('pdf_autofill_presets') || '[]');
...
function saveAutoFillPreset() {
    const name = DOMElements.afPresetName.value.trim();
    if (!name) { alert("プリセット名を入力してください。"); return; }
    
    const rules = [];
    DOMElements.afRulesContainer.querySelectorAll('.rule-row').forEach(row => {
        const k = row.querySelector('.af-key').value.trim();
        const v = row.querySelector('.af-val').value.trim();
        if (k !== '') rules.push({ key: k, value: v });
    });
    
    if (rules.length === 0) { alert("保存するルールがありません。"); return; }

    const presets = JSON.parse(localStorage.getItem('pdf_autofill_presets') || '[]');
    presets.push({ name, rules });
    localStorage.setItem('pdf_autofill_presets', JSON.stringify(presets));
    
    alert(`プリセット「${name}」を保存しました。`);
    DOMElements.afPresetName.value = '';
    loadAutoFillPresets();
    
    // Select the newly saved preset
    DOMElements.afPresetSelect.value = presets.length - 1;
}

function applyAutoFillPreset() {
    const index = DOMElements.afPresetSelect.value;
    if (index === "") return;
    const presets = JSON.parse(localStorage.getItem('pdf_autofill_presets') || '[]');
    const preset = presets[index];
    if (preset) {
        DOMElements.afRulesContainer.innerHTML = '';
        preset.rules.forEach(r => addAutoFillRuleRow(r.key, r.value));
    }
}

function deleteAutoFillPreset() {
    const index = DOMElements.afPresetSelect.value;
    if (index === "") return;
    if (!confirm("本当にこのプリセットを削除しますか？")) return;
