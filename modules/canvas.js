...
function drawFixedBox(x, y, fieldId, width, height, pos) {
    ...
    // --- Placement/Edit Mode Styles ---
    const selectionColor = "#1a73e8"; // Google Blue
    const hoverFillColor = "rgba(26, 115, 232, 0.1)";
    ...
    if (isHovered && !isSelected) {
        ...
        // Normal state: Light green fill and border
        ctx.fillStyle = "rgba(34, 191, 71, 0.31)"; // Light green fill
        ctx.fillRect(x, y, width, height);
        ctx.strokeStyle = "rgba(40, 167, 69, 0.5)"; // Medium green border
        ctx.lineWidth = 1;
        ctx.setLineDash([]); 
    ...
    if (pos.type === 'textarea') {
        ctx.textBaseline = "top"; ctx.textAlign = "left";
        const previewText = pos.value || "文章欄";
        ...
        const previewText = pos.value || "テキスト";
        ...
        if (totalTextWidth > width) {
            ctx.textAlign = "left"; 
            ctx.fillText(previewText, x, y + height / 2);
        } else {
            ctx.textAlign = "center"; 
            ctx.fillText(previewText, x + width / 2, y + height / 2);
        }
        ctx.restore();
    } else if (pos.type === 'circle' || pos.type === 'check') {
        drawMarker(ctx, x, y, width, height, pos.type, isSelected ? selectionColor : 'rgba(40, 167, 69, 0.8)');
    }
    ...
    // 4. Label (Project Name)
    const labelFontSize = 11 * (canvas.width / state.pdfPage.originalWidth);
    ctx.fillStyle = isSelected ? selectionColor : "#888"; 
    ctx.font = `${labelFontSize}px sans-serif`;
    ctx.textBaseline = "bottom"; ctx.textAlign = "left"; 
    ctx.fillText(pos.label, x, y - 3);
}
